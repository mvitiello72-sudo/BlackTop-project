...
public class AggiungiIndirizzoServlet extends HttpServlet
{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		// Il checkbox invia "true" se selezionato, altrimenti restituisce null
			...
			codicePostale != null && !codicePostale.trim().isEmpty())
		{
			try
			{
				...
				response.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Impossibile salvare l'indirizzo.");
				return;
			}
		}
		response.sendRedirect(request.getContextPath() + "/common/profilo");
